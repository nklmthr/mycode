...
public interface MonthlyBalanceRepository extends JpaRepository<MonthlyBalance, String> {
	...
	@Query("select new com.nklmthr.finance.personal.dao.MonthlyBalanceSummaryDTO(mb.date, mb.account.accountType.classification, sum(mb.amount)) from MonthlyBalance mb where year(mb.date) = ?1 and month(mb.date) = ?2 group by mb.date, mb.account.accountType.classification order by mb.date desc ")
