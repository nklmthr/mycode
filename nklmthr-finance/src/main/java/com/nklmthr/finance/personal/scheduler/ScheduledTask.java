...
public abstract class ScheduledTask  {
		...
		if (messages.isEmpty()) {
			...
			for (Message mess : messages) {
				...
				Transaction transaction = transactionService.findTransactionsBySource(message.getThreadId(),
						getRecievedTime(message).getTime());
				if (transaction == null) {
					logger.info("Transaction not found in database.. Adding Transaction:" + message.getThreadId()
							+ ", sourceTime:" + getRecievedTime(message).getTime() + ", Time:"
							+ new Date(getRecievedTime(message).getTime()));
					String email;
					try {
					...
					if (transaction != null) {
						...
						transaction.setSourceTime(getRecievedTime(message).getTime());
						transaction.setDate(getRecievedTime(message));
	...
	private Date getRecievedTime(Message message) throws JSONException, IOException, ParseException {

		String recievedTime = message.getPayload().getHeaders().stream().filter(s -> s.getName().equals("Date"))
				.map(s -> s.getValue()).collect(Collectors.joining());
		// Sat, 29 Jun 2024 11:51:14 +0530
		SimpleDateFormat dateFormat = new SimpleDateFormat("E, dd MMM yyyy HH:mm:ss Z");
		Date recievedDate = dateFormat.parse(recievedTime);
		logger.debug("recievedTime:" + recievedTime + ", " + recievedDate.getTime());
		return recievedDate;
	}
	...
	protected abstract String getGMailAPIQuery();
