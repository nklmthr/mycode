...
public class TransactionUploader {
	...
	public static void main(String args[]) throws URISyntaxException {
		List<TransactionCategory> result = uploadCategories("categories.csv");
		System.out.println(result);
		updateCategoriesToDB(result);
	}
	
	
	private static void updateCategoriesToDB(List<TransactionCategory> result) {
		// TODO Auto-generated method stub
		
	}


	private static List<TransactionCategory> uploadCategories(String file) throws URISyntaxException {
		List<TransactionCategory> result = new ArrayList();
		Path path = Paths.get(ClassLoader.getSystemResource(file).toURI());
		try (Reader reader = Files.newBufferedReader(path)) {
			CsvToBean<TransactionCategory> cb = new CsvToBeanBuilder<TransactionCategory>(reader)
					.withOrderedResults(true).withType(TransactionCategory.class).build();
			Iterator<TransactionCategory> itr = cb.iterator();
	...
	public static void transactionsUpdate(String[] args) throws Exception {
	...
	private static void updateTOdatabase(List<Transaction> transactions) {
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mydb", "root", "adapter");
